...
final class GarantPresenter extends Nette\Application\UI\Presenter 
{
	...
	public function createComponentTaskStudentsGrid($name)
	{
		...
		//$grid->setDataSource($this->database->table("student_has_task")->where("id_task", $this->id_task)->select("user->email,user->first_name,user->surname,points"));
		$grid->setDataSource($this->database->table("user")->select("user.*,student_has_task.points"));

...
final class GarantPresenter extends Nette\Application\UI\Presenter 
{
	...
	public function createComponentLectorsAdd($name)
	{
		...
		$grid->setDataSource($this->database->query("SELECT id_user, email, first_name, surname FROM user WHERE rank >= 2 AND id_user NOT IN (SELECT id_user FROM user NATURAL LEFT JOIN course_has_lecturer WHERE rank >= 2 AND id_course = ?) AND id_user NOT IN (SELECT id_user FROM user NATURAL JOIN course_has_student)", $this->id_course)->fetchAll());

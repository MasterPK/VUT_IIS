...
final class ChiefPresenter extends Nette\Application\UI\Presenter
{
	...
	public function createComponentManageEquipmentGrid($name)
	{
		...
		$grid->setDataSource($this->database->table("room_has_equipment")->select("room_has_equipment.*,id_room_equipment.id_room_equipment"));
	...
	{
		$data = $this->database->query("SELECT * FROM room_equipment NATURAL JOIN room_equipment WHERE id_room = ?",  $id_room)->fetchAll();
